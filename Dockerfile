# Imagine we start with a empty machine (container) here, each command below make some changes to the machine

# downloading all the necessary software to run maven, (search this maven base image in dockerHub to see what is included)
...
# `cd` into the app folder inside the virtual machine
WORKDIR /app

# copy war file generated earlier into the tomcat webapps folder inside the container
COPY --from=builder /app/target/project5.war /usr/local/tomcat/webapps/project5.war
